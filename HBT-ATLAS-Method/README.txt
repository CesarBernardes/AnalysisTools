...
1) For HIJING
log(B_A_same-sign) = (0.996 +- 0.017)*log(B_A_opp-sign) + (-0.247 +- 0.034)
sigma_B,A_same-sign = (1.211 +- 0.219)*sigma_B,A_opp-sign + (-0.062 +- 0.062)

2) For Pythia
log(B_A_same-sign) = (1.209 +- 0.041)*log(B_A_opp-sign) + (0.190 +- 0.098)
...
Convert these parameters to same sign pairs by using the functions above (Important: note that for B_A the relation above is with log, so you need to convert it). You can have results for Pythia and Hijing.
